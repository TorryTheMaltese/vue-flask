var kurly_common = (function(){
    ...
    // 이미지교체
    // function tabOff(target,before,after){
    //     if(target.attr("src").indexOf("_on") != -1){
    //         var menuName = target.attr("src").replace(before,after);
    //         target.attr("src",menuName);
    //     };
    // };
    // function tabOn(target,before,after){
    //     if(target.attr("src").indexOf("_on") == -1){
    //         var menuName = target.attr("src").replace(before,after);
    //         target.attr("src",menuName);
    //     };
    ...
    function _qnb(target){
        // 스크롤시 qnb고정
        var qnbPos = parseInt( $("#qnb").css('top') );
...
//		if( jQuery(".goods-list-position a").last().text() == "Daily" || jQuery(".goods-list-position a").last().text() == "장바구니 필수품" || jQuery(".goods-list-position a").last().text() == "KURLY PASS" ){
