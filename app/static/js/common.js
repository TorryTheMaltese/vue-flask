...
if (typeof thefarmers !== "undefined") {
    var _oldAlert = window.alert;
    window.alert = function () {
        thefarmers.alert.apply(thefarmers, arguments);
    };
}
...
function chkForm(form)
{
    ...
    if (form.password2){
        if (form.password.value!=form.password2.value){
            thefarmers.alert("비밀번호가 일치하지 않습니다");
            form.password.value = "";
    ...
    if (document.getElementById('avoidDbl')) document.getElementById('avoidDbl').innerHTML = '<p class="txt_operate"><span>처리중입니다 ▶</span></p>';
    return true;
}
...
function chkLength(field,len)
{
    ...
    if (text.trim().length<len){
        thefarmers.alert(len + "자 이상 입력하셔야 합니다", function () {
            field.focus();
        });
        return false;
    }
...
{
    var ret = false;
    var fieldname = form.elements[field.name];
    if (fieldname.length){
    ...
    if (!ret){
        ...
        if (msg) msg2 += "\n\n" + msg;
        thefarmers.alert(msg2, function () {
            field.focus();
        });
        return false;
    }
...
function chkPatten(field,patten,msg)
{
    ...
    var regId			= /^[a-zA-Z0-9]{1}[^'"ㄱ-ㅎㅏ-ㅣ\uAC00-\uD7A3]{3,15}$/; //아이디 체크에서 한글 막기 (2018-02-01, 박소민)
...
{
    var ret = false;
    var fieldname = form.elements[field];//eval("form.elements['"+field+"']");
    if (fieldname.length){
...
function chkResno(form)
{
    ...
    fmt = /^\d{6}[1234]\d{6}$/;
    if (!fmt.test(resno)) {
        thefarmers.alert('잘못된 주민등록번호입니다.'); return false;
    }
    ...
    if ( birth.getYear()%100 != resno.substr(0, 2) || birth.getMonth() != birthMonth || birth.getDate() != birthDate) {
        thefarmers.alert('잘못된 주민등록번호입니다.');
        return false;
    }
...
function chg_cart_ea(obj,str,idx)
{
    ...
    var min = parseInt(obj.getAttribute('min')) || 0;
    var max = parseInt(obj.getAttribute('max')) || 0;

    if (isNaN(obj.value) || obj.value == '') {
        thefarmers.alert ("구매수량은 숫자만 가능합니다", function () {
            obj.value=step;
            obj.focus();
        });
...
function layerCartAdd(frm){
    var goodsName = '';
    var goodsNameItem = [];
    if($('[name=packageCheck]').val() === 'true'){
        $('#el-multi-option-display .goods-view-order-item').each(function(){
            goodsNameItem.push($(this).find('.goods-view-order-item-name').text().split('/')[1]);
        });
        ...
        goodsName = $('[name=frmView]').find('.goods-view-name').text();
    }
    // end
...
function ajaxCart (param, callback) {
    var ajaxUrl= "/shop/goods/ajax_cartAdd.php";
    jQuery.ajax({
        url : ajaxUrl,
        type : 'POST',
        data : param,
...
function cartAdd(form,redirectType)
{
    ...
    if (redirectType=='Direct')
    {
        ...
        layerCartAdd(form, redirectType);	//장바구니 레이어 열기
    }
...
function review_like(r_no,$self){
    if(review_like_check) return;
    review_like_check = true;
    if(!GD_ISMEMBER) {
        if(confirm("회원전용 서비스 입니다. 로그인/회원가입 페이지로 이동하시겠습니까?")) {
            if(typeof(parent) != "undefined") {
                parent.location.href = "/shop/member/login.php?return_url="+encodeURIComponent(parent.location.href);
            } else {
                document.location.href = "/shop/member/login.php?return_url="+encodeURIComponent(location.href);
            }
        }
        ...
        $.ajax({
            type: "POST",
            url: '../goods/ajax_review_like.php',
            dataType: 'json',
            data : {"mode" : "add_like", "r_no" : r_no},
...
function review_addhit(sno){
    $.ajax({
        type: "POST",
        url: '../goods/ajax_review_addhit.php',
        dataType: 'json',
        data : { "sno" : sno },
        success: function(res) {
            if(res.hit){
...
function address_chk_popup() {
    popup('../proc/popup_address2.php',530,500);	// 고도몰 우편번호(2016.08.31, 김기완)
    /*/
        new daum.Postcode({
            animation:true,
            oncomplete: function(data) {
                $.ajax({
                    type : 'POST',
                    url : '../proc/ajax_address_chk.php',
                    data : { 'address' : data.autoJibunAddress ? data.autoJibunAddress : data.jibunAddress, 'address_sub' : '', 'zipcode' : data.postcode, 'zonecode' : data.zonecode },
                    dataType:"json",
